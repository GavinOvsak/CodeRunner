...
export const CR_MED_BY_KEY: Record<string, Med> = Object.fromEntries(CR_MEDS.map(m => [m.key, m]));
...
export function crNextTasks(s: Patient): NextTask[] {
      ...
      if (s.pulse === 'Yes') {
        ...
        if (s.rhythm === 'VF' || s.rhythm === 'VT') {
          ...
          if (shockCount >= 2) {
            ...
            const amioLabel = amioGiven === 0 ? 'Give Amiodarone 300mg' : 'Give Amiodarone 150mg';
        ...
        push({ id: 'resume-cpr', label: 'Resume CPR (next cycle)', kind: 'critical' });
      }
    } else {
  ...
  if (s.pulse === 'Yes' && s.rate === 'Fast' && s.symptomatic === 'Yes') {
    ...
    if (s.rhythm === 'WideTach') {
      const amioGiven = crGiven(s, 'amio');
      const amioLabel = amioGiven === 0 ? 'Amiodarone 150mg over 10 min' : 'Amiodarone 150mg (repeat)';
      push({ id: 'amio', label: amioLabel, kind: 'med', medKey: 'amio' });
    }
    if (s.rhythm === 'Afib' || s.rhythm === 'Aflutter')
      push({ id: 'cardiovert', label: 'Synchronized Cardioversion (if unstable)', kind: 'shock' });
  }
  ...
  if (s.pulse === 'Yes' && s.rate === 'Slow' && s.symptomatic === 'Yes') {
    ...
    push({ id: 'dopamine',  label: 'Dopamine gtt',          kind: 'med', medKey: 'dopamine' });
  }
...
export function crRecommendedMedKeys(s: Patient): Set<string> {
  ...
  tasks.forEach(t => { if (t.medKey) keys.add(t.medKey); if (t.kind === 'shock') keys.add('shock'); });
  return keys;
}
