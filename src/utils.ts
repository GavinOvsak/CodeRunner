...
export function reconstructStateFromLog(p: Patient): Patient {
  const base: Patient = {
    ...
    symptomatic: '?',
    rhythm: '?',
    cpr: { active: false, cycleNumber: 0, cycleStartAt: 0, pausedAt: null, metronomeAnchor: p.startedAt },
    gave: p.gave.filter(g => g.doses.length === 0).map(g => ({ key: g.key, doses: [] })),
    doneTasks: {},
    log: p.log,
  };
  ...
  for (const entry of sortedLog) {
    ...
    if (type === 'status') {
      ...
      if (alertMatch) {
        ...
        if ((nextAlert === 'No' || nextAlert === 'Altered') && base.alert === 'Yes') {
          base.pulse = '?';
        }
      ...
      const breathingMatch = /^Breathing:\s*(Yes|No|ETT)/i.exec(text);
      if (breathingMatch) {
      ...
      if (rhythmMatch) {
        ...
        else if (rStr === 'Sinus Brady' || rStr === 'SinusBrady') rhythmVal = 'SinusBrady';
        else if (rStr === 'Wide VT' || rStr === 'WideTach') rhythmVal = 'WideTach';
        else if (rStr === 'A-Flutter' || rStr === 'Aflutter') rhythmVal = 'Aflutter';
        else if (rStr === 'A-Fib' || rStr === 'Afib') rhythmVal = 'Afib';
        else if (['VT', 'VF', 'PEA', 'Asystole', 'NSR', 'SVT', '?'].includes(rStr)) rhythmVal = rStr as RhythmValue;
      ...
      if (text.startsWith('CPR started')) {
        ...
        const cycleNum = cycleMatch ? parseInt(cycleMatch[1], 10) : base.cpr.cycleNumber + 1;
        base.cpr = {
          active: true,
        ...
        const cycleMatch = /cycle\s+(\d+)/i.exec(text);
        const cycleNum = cycleMatch ? parseInt(cycleMatch[1], 10) : base.cpr.cycleNumber;
        base.cpr = {
          ...base.cpr,
          cycleNumber: cycleNum,
          pausedAt: entry.at,
      ...
      if (text === 'Airway secured (ETT)' || text === '✓ Airway → advanced (ETT)') {
        base.breathing = 'ETT';
        base.alert = 'Sedated';
        base.doneTasks['airway__hidden'] = true;
      } else if (text === 'IV/IO access obtained' || text === '✓ Obtain IV / IO Access') {
        base.doneTasks['access'] = true;
        base.doneTasks['access__hidden'] = true;
      } else if (text === 'AED requested' || text === '✓ Get AED / Defibrillator') {
        ...
        const labelMap: Record<string, string> = {
          ...
          'ROSC — End Code': 'rosc',
          'Resume CPR (next cycle)': 'resume-cpr',
          'Start CPR': 'start-cpr',
          'Give Epi 1mg': 'epi',
          ...
          'Synchronized Cardioversion (if unstable)': 'cardiovert',
